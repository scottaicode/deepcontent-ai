...
const platformToContentType = {
  // 'website': 'article', // Removed website as it doesn't provide real value yet
  'blog': 'blog-post',
  'social': 'social-post',
  'email': 'email',
...
// Add a mapping for subplatform to content type
const subPlatformToContentType: Record<string, string> = {
  // Blog subplatforms
  'medium': 'blog-post',
  'wordpress': 'blog-post',
  ...
  'sales-presentation': 'presentation', // Renamed to avoid duplicate key
...
export const ContentForm: React.FC<ContentFormProps> = ({ 
  ...
  const getContentType = () => {
    ...
    if (subPlatform) {
      if (platform === 'social') {
        ...
        return `${subPlatform}-${platformToContentType[platform as keyof typeof platformToContentType] || ''}`;
      }
    }
  ...
  const handleSubPlatformSelect = (subPlatformId: string) => {
    ...
    if (platform === 'social') {
      ...
      derivedContentType = `${subPlatformId}-${platformToContentType[platform as keyof typeof platformToContentType]}`;
    }
  ...
  const togglePlatformExpansion = (platformId: string) => {
    ...
    if (expandedPlatform === platformId) {
      ...
      const baseContentType = platformToContentType[platformId as keyof typeof platformToContentType] || 'article';
            ...
            <div className="mt-4">
              <div className="flex items-center text-blue-600 text-sm mb-2">
                ...
                <span className="font-medium">
                  {documentContent.fileName} ({Math.round(documentContent.fileSize / 1024)} KB)
                </span>
              </div>
              ...
              <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium">
                    {t('createPage.documentContentExtracted', { defaultValue: 'Document content extracted for research' })}
                  </span>
                  <button
                    ...
                    aria-label={showFullDocument ? "Collapse document content" : "Expand document content"}
                  >
                    {showFullDocument ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                  </button>
                </div>
                
                {showFullDocument && (
                  <div className="mt-2 text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap max-h-60 overflow-y-auto p-2 bg-gray-50 dark:bg-gray-800 rounded transition-all duration-300">
  ...
  const handleWebsiteContent = (content: any) => {
    console.log('Website content received:', content);
    setWebsiteContent(content);
    
    // If there's a title in the content, suggest it as a business name
    if (content.title && !businessType) {
      // Extract potential business name from title
      const potentialName = content.title.split(' - ')[0].split(' | ')[0].trim();
      if (potentialName) {
        setBusinessType(potentialName);
        safeToast(t('createPage.businessNameExtracted', { defaultValue: 'Business name extracted from website' }), 'success');
      }
    }
    
    // If there's a company description, suggest it for the audience needs
    if (content.aboutContent && !audienceNeeds) {
      setAudienceNeeds(content.aboutContent.substring(0, 500));
    }
  };
