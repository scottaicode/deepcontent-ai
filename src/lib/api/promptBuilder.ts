...
// Content type detection to determine if this is a business, personal brand, or hobbyist
...
I have performed a detailed scrape of ${companyName ? companyName + "'s" : "the creator's"} website and collected the following information. This information is AUTHORITATIVE and MUST be used as the PRIMARY SOURCE for your research:
...
About Content: 
"${websiteContent.aboutContent?.substring(0, 800) || 'Not available'}"

${websiteContent.productInfo ? `Product/Service Information:
"${websiteContent.productInfo?.substring(0, 800) || 'Not available'}"` : ''}

${websiteContent.pricingInfo ? `Pricing Information:
"${websiteContent.pricingInfo?.substring(0, 800) || 'Not available'}"` : ''}
...
Key Paragraphs From The Website (MUST INCORPORATE THESE):
${websiteContent.paragraphs?.slice(0, 15).map((p: string, i: number) => `[Paragraph ${i+1}]: "${p.substring(0, 300)}${p.length > 300 ? '...' : ''}"`).join('\n\n') || 'None found'}

CRITICAL REQUIREMENT: You MUST directly quote from and cite this scraped website data in your research. For every section in your research, include at least 2-3 direct references to this website data using the format: "According to ${companyName ? companyName + "'s" : "the creator's"} website: [direct quote]"
...
CRITICAL REMINDER: THE USER-PROVIDED DATA (website content, transcript, image analysis, follow-up answers) MUST BE YOUR PRIMARY SOURCES. They are verified, current, and directly relevant to the research needs.
