...
export async function GET(request: NextRequest) {
  try {
    ...
    // We have a valid API key, so let's use it
    console.log('Using Supadata API with the provided key');
    ...
    try {
      ...
      if (!data) {
        console.log('API calls failed after retries - falling back to simulation');
        data = getSimulatedTranscriptData(videoId);
      }
    } catch (error: any) {
      console.error('Error fetching transcript:', error);
      // Only fall back to simulation in development mode
      if (process.env.NODE_ENV === 'development') {
        console.log('API error - falling back to simulation in development mode');
        data = getSimulatedTranscriptData(videoId);
      } else {
        // In production, let the error propagate to the client
        throw error;
      }
    }
...
async function fetchWithRetry(videoId: string, videoUrl: string, attempt = 0): Promise<any> {
  try {
    ...
    // According to documentation, the API expects an x-api-key header
    // regardless of the API key format
    if (SUPADATA_API_KEY) {
      headers['x-api-key'] = SUPADATA_API_KEY;
      console.log('Using x-api-key header as per documentation');
    }
    ...
    const response = await fetch(apiUrl.toString(), {
      ...
      signal: AbortSignal.timeout(15000) // 15 second timeout (increased for potentially slow API)
    });
