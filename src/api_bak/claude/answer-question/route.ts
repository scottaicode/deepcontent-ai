  ...
  try {
    ...
    if (!anthropicApiKey) {
      ...
      return NextResponse.json({ 
        answer: simulateAnswer(question, language),
...
/**
 * Generate simulated answer when API key is not available
 */
function simulateAnswer(question: string, language: string): string {
  if (language === 'es') {
    return `Esta es una respuesta simulada a la pregunta "${question}" ya que la clave API de Claude no está configurada. Para obtener respuestas generadas por IA, por favor configura tu clave API de Anthropic en las variables de entorno.`;
  }
  
  // Default English simulated answer
  return `This is a simulated answer to the question "${question}" as the Claude API key is not configured. For AI-generated answers, please set up your Anthropic API key in the environment variables.`;
} 