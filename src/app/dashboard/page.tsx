...
export default function DashboardPage() {
  ...
  // Clean up and simplify the status info component
  const renderStatusInfo = () => {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4 mb-6 text-sm">
        <h3 className="font-medium text-gray-900 dark:text-white mb-2">{t('dashboard.systemStatus')}</h3>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-gray-600 dark:text-gray-400">{t('dashboard.userAuthenticated')}</span>
          ...
          <div className="flex items-center justify-between">
            <span className="text-gray-600 dark:text-gray-400">{t('dashboard.databaseConnection')}</span>
            <span className="flex items-center">
          ...
          <div className="flex items-center justify-between">
            <span className="text-gray-600 dark:text-gray-400">{t('dashboard.lastRefresh')}</span>
            <span>
              {lastRefreshTime ? new Intl.DateTimeFormat(undefined, {
                hour: 'numeric',
                minute: 'numeric',
                second: 'numeric',
                hour12: true
              }).format(lastRefreshTime) : t('dashboard.never')}
            </span>
          </div>
        </div>
      </div>
